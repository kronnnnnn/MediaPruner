...
class LibraryPath(Base):
    ...
    # Relationships
    movies = relationship("Movie", back_populates="library_path", cascade="all, delete-orphan")
    tvshows = relationship("TVShow", back_populates="library_path", cascade="all, delete-orphan")
...
class Movie(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    library_path_id = Column(Integer, ForeignKey("library_paths.id"), nullable=False)
    
    # File info
    file_path = Column(String(1024), nullable=False, unique=True)
    file_name = Column(String(512), nullable=False)
    folder_name = Column(String(512))  # Parent folder name for scraping fallback
    ...
    rotten_tomatoes_score = Column(Integer)  # Rotten Tomatoes Tomatometer (0-100)
    rotten_tomatoes_audience = Column(Integer)  # Rotten Tomatoes Audience Score (0-100)
    ...
    video_hdr = Column(String(32))  # HDR format if present (HDR10, Dolby Vision, etc.)
    ...
    subtitle_path = Column(String(1024))  # Path to associated external subtitle file
...
class TVShow(Base):
    ...
    seasons = relationship("Season", back_populates="tvshow", cascade="all, delete-orphan")
