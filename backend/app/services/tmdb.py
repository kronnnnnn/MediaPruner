...
def get_full_image_url(path: Optional[str], size: str = "w500") -> Optional[str]:
...
class TMDBService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.use_bearer = self.api_key.startswith("eyJ") if self.api_key else False
    ...
    async def get_movie_details(self, tmdb_id: int) -> Optional[TMDBMovieResult]:
    ...
    async def find_movie_by_tmdb_id(self, tmdb_id: int) -> Optional[TMDBMovieResult]:
        """Get movie by TMDB ID directly"""
        return await self.get_movie_details(tmdb_id)

    async def search_tvshow(self, query: str, year: Optional[int] = None) -> list[dict]:
        ...
        if results:
            logger.debug(f"TMDB TV search top results: {[r.get('name', 'Unknown') for r in results[:3]]}")
        return results
    
    async def get_tvshow_details(self, tmdb_id: int) -> Optional[TMDBTVShowResult]:
    ...
    async def get_season_details(self, tmdb_id: int, season_number: int) -> list[TMDBEpisodeResult]:
