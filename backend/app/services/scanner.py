...
EPISODE_ONLY_PATTERNS = [
    ...
    re.compile(
        r'(?:^|[\s\._-])(?:[Ee]p?|[Ee]pisode)[\s\._-]*(\d{1,3})(?:[\s\._-]|$)',
        re.IGNORECASE),
    # Part 1, Part.1, Pt1, Pt.1
    re.compile(
        r'(?:^|[\s\._-])(?:[Pp]art|[Pp]t)[\s\._-]*(\d{1,3})(?:[\s\._-]|$)',
        re.IGNORECASE),
]
...
# Fallback pattern for season/episode as 3-4 digit number (risky, only use
# as last resort)
EPISODE_NUMERIC_PATTERN = re.compile(
    r'(?:^|[\s\._-])(\d{1,2})(\d{2})(?:[\s\._-]|$)')
...
CLEAN_PATTERNS = [
    ...
    re.compile(
        r'(?:720p|1080p|2160p|4[kK]|[hH][dD][rR]|[bB]lu[rR]ay|[wW][eE][bB]-?[dD][lL]|[dD][vV][dD]|[xX]264|[xX]265|[hH][eE][vV][cC]|[aA][aA][cC]|[aA][cC]3).*',
        re.IGNORECASE),
...
def parse_episode_filename(
        file_path: Path,
        show_folder: str = "",
        season_from_folder: Optional[int] = None) -> Optional[ParsedEpisode]:
    """
    ...
    season = None
    episode = None
    # Track where the match occurred for title extraction
    match_pos = len(file_name)
    ...
    # If we still don't have an episode number, check if season from folder
    # helps
    if episode is None:
        return None

    # If we have episode but no season, use folder-derived season or default
    # to 1
    if season is None:
        season = season_from_folder if season_from_folder is not None else 1
...
def _scan_movie_subfolder(
        folder: Path,
        movie_folder_name: str) -> list[ParsedMovie]:
    """
    ...
    if all_videos:
        ...
        # Use folder name if it looks like a movie name (has year), otherwise
        # use filename
        if MOVIE_YEAR_PATTERN.search(movie_folder_name):
            parsed = parse_movie_filename(
                Path(movie_folder_name + largest_file.suffix))
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for f in files:
            ...
            parsed = parse_episode_filename(
                file_path, show_folder, season_from_folder)
            if parsed is None:
                # If standard parsing failed, this file might not be a TV
                # episode
                continue
