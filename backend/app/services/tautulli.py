...
class TautulliService:
    ...
    async def _make_request(self, cmd: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    async def search_movie_history(self, title: Optional[str], year: Optional[int] = None, imdb_id: Optional[str] = None, db=None) -> tuple[list[Dict[str, Any]], Optional[int]]:
        """
        ...
        for query in queries:
            logger.debug(f"Tautulli: searching for movie with query='{query}' (imdb_id={imdb_id}, year={year})")
            search_results = await self._make_request("search", {"query": query})

            # Log a brief summary for debugging if the search returned no results
            if not search_results or "results" not in search_results:
                logger.debug(f"Tautulli: no search results for query='{query}'")
                # If no results, try scanning recent history directly (some servers don't index guids for search)
                if imdb_id:
                    try:
                        logger.debug("Tautulli: scanning recent history for imdb_id as fallback when search returns no results")
                        ...
                        if matches:
                            ...
                            # Try to extract rating_key from matched history entries
                            rk = None
                            for e in matches:
                                if e.get('rating_key'):
                                    try:
                            ...
                            if plex_results:
                                for pr in plex_results:
                                    ...
                                    if rk:
                                        try:
                                            rk_int = int(rk)
                                            logger.debug(f"Tautulli: resolved rating_key={rk_int} via Plex title search fallback for title='{title}'")
                                            history = await self.get_history(rating_key=rk_int)
                                            return history, rk_int
                                        except Exception:
                                            continue
                    except Exception as e:
                        logger.debug(f"Tautulli: Plex fallback after empty search failed: {e}")
            ...
            if imdb_id and movies:
                ...
                if imdb_match:
                    ...
                    logger.debug(f"Tautulli: found imdb match, rating_key={rating_key} for imdb_id={imdb_id}")
                    if rating_key:
                        history = await self.get_history(rating_key=rating_key)
                        return history, int(rating_key)
                    else:
                        logger.debug(f"Tautulli: imdb match found but no rating_key present in result: {imdb_match}")
            ...
            # may present matches outside the 'movie' media_type or omit media_type).
            if imdb_id:
                for r in results:
                    try:
                    ...
                    if imdb_id in text:
                        ...
                        if rating_key:
                            ...
                            logger.debug(f"Tautulli: found imdb id in result but no rating_key present: {r}")

            # If still no rating_key found, attempt to resolve rating_key via Plex (if configured)
            if imdb_id and db is not None:
                try:
                    ...
                    if plex:
                        ...
                        if rating_key:
                            logger.debug(f"Tautulli: resolved rating_key={rating_key} via Plex for imdb_id={imdb_id}")
                            history = await self.get_history(rating_key=rating_key)
                            return history, int(rating_key)
                        # If Plex couldn't resolve via imdb_id, try searching Plex by title as a fallback
                        if title:
                            try:
                                plex_results = await plex.search(title)
                                if plex_results:
                                    # Prefer movie-type results and grab ratingKey if present
                                    for pr in plex_results:
                                        rk = pr.get('ratingKey') or pr.get('rating_key') or pr.get('ratingkey')
                                        if rk:
                                            try:
                                                rk_int = int(rk)
                                                logger.debug(f"Tautulli: resolved rating_key={rk_int} via Plex title search for title='{title}'")
                                                history = await self.get_history(rating_key=rk_int)
                                                return history, rk_int
                                            except Exception:
                                                continue
                            except Exception as e:
                                logger.debug(f"Tautulli: Plex title search failed: {e}")
                except Exception as e:
            ...
            # 1) Use the first movie search result's rating_key (title-based match)
            if movies:
            ...
            if imdb_id:
                try:
                    logger.debug("Tautulli: scanning recent history for imdb_id/title matches as fallback")
                    recent = await self.get_history(length=2000)
                    matches = []
                    for entry in recent:
                        try:
    ...
    async def search_tvshow_history(
        ...
        if season_number is not None:
            history = [h for h in history if h.get("parent_media_index") == season_number]
        
        if episode_number is not None and season_number is not None:
            history = [h for h in history if h.get("media_index") == episode_number]
        
        return history
    
    async def get_library_watch_time_stats(self, section_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    async def get_user_watch_time_stats(self, user_id: Optional[int] = None) -> Dict[str, Any]:
...
async def get_tautulli_service(db) -> Optional[TautulliService]:
    ...
    try:
        ...
        logger.info(f"Tautulli settings found - host: {host_val}, api_key_present: {key_val_present}")
    except Exception:
