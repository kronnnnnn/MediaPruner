...
def mux_subtitle_into_video(
    ...
    """
    if not is_available():
        return MuxResult(success=False, error="FFmpeg is not installed or not in PATH")
    
    if not video_path.exists():
        return MuxResult(success=False, error=f"Video file not found: {video_path}")
    
    if not subtitle_path.exists():
        return MuxResult(success=False, error=f"Subtitle file not found: {subtitle_path}")
    ...
    if subtitle_title:
        ...
        lang_names = {
            ...
            'kor': 'Korean', 'chi': 'Chinese', 'ara': 'Arabic', 'dut': 'Dutch',
    ...
    try:
        ...
        if delete_originals:
            # Delete original video if it still exists and is different from output
        ...
        if temp_output and temp_output.exists():
            temp_output.unlink()
        return MuxResult(success=False, error="FFmpeg timed out after 10 minutes")
